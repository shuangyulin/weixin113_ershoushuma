<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="发送内容" prop="fasongneirong">
          <el-input v-model="ruleForm.fasongneirong" 
              placeholder="发送内容" clearable  :readonly="ro.fasongneirong"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="发送内容" prop="fasongneirong">
              <el-input v-model="ruleForm.fasongneirong" 
                placeholder="发送内容" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tongxin/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tongxinCrossAddOrUpdateFlag = false;
                  this.parent.search();
